...
def parse_file( fname, edges, transform, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == 'line':
            ...
            print(f"CIRCLE {str(args)}")
            add_circle(edges,float(args[0]),float(args[1]),float(args[2]),float(args[3]),float(100))
        elif line == 'hermite' or line == "bezier":
            print(f"HERM OR BEZ {str(args)}")
            ...
            if line == 'display':
                print(f"DISPLAY\t{str(args)}")
                display(screen)
            else:
                print(f"SAVE\t{str(args)}")
